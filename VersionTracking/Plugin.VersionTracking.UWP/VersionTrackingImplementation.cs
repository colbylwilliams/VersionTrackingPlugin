...
namespace Plugin.VersionTracking
{
    ...
    public class VersionTrackingImplementation : IVersionTracking
    {
        ...
        public void Track()
        {
            ...
            {
                var oldVersionList = JsonConvert.DeserializeObject<List<string>>(AppSettings.Values[xamVersionsKey] as string);

                var oldBuildList = JsonConvert.DeserializeObject<List<string>>(AppSettings.Values[xamBuildsKey] as string);
            ...
            if (needsSync)
            {
                ...
                lock (locker)
                {
                    ...
                    AppSettings.Values[xamVersionsKey] = JsonConvert.SerializeObject(versionTrail[xamVersionsKey]);
                    AppSettings.Values[xamBuildsKey] = JsonConvert.SerializeObject(versionTrail[xamBuildsKey]);
                }
            }
        }
