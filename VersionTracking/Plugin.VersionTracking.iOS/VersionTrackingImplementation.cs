...
namespace Plugin.VersionTracking
{
    ...
    public class VersionTrackingImplementation : IVersionTracking
    {
        ...
        public void Track()
        {
            ...
            // load history
            var nsOldVersionTrail = NSUserDefaults.StandardUserDefaults.DictionaryForKey(xamUserDefaultsVersionTrailKey);

            //check if its the first ever launch
            if (nsOldVersionTrail == null) {
                ...
                foreach (var item in nsOldVersionTrail) {

                    var key = item.Key.ToString();
                    var val = item.Value as NSArray;
                    ...
                    if (key != null && val != null) {
                        ...
                        for (int i = 0; i < (int)val.Count; i++) {
                            list.Add(val.GetItem<NSString>((nuint)i).ToString());
                        }

                        versionTrail.Add(key, list);
                    }
                }
            ...
            if (needsSync) {
                ...
                var nsDict = NSDictionary<NSString, NSArray>.FromObjectsAndKeys(
                    versionTrail.Values.Select(l => new NSMutableArray<NSString>(l.Select(s => new NSString(s)).ToArray())).ToArray(),
                    versionTrail.Keys.Select(s => new NSString(s)).ToArray());

                NSUserDefaults.StandardUserDefaults[xamUserDefaultsVersionTrailKey] = nsDict;
        ...
        public string CurrentVersion => NSBundle.MainBundle.ObjectForInfoDictionary("CFBundleShortVersionString").ToString();
        ...
        public string CurrentBuild => NSBundle.MainBundle.ObjectForInfoDictionary("CFBundleVersion").ToString();
